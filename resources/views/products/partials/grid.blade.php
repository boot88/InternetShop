    ...
    @foreach($products as $product)
      ...
      <article class="group overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md">
        ...
        <div class="px-3 pb-3 sm:px-4 sm:pb-4">
          <p class="mb-3 text-xs text-slate-500">{{ $product->in_stock ? 'Доставка после подтверждения' : 'Сообщим о поступлении' }}</p>
          <div class="flex items-center justify-between gap-2">
            <div class="min-w-0"><span class="text-base font-semibold text-slate-950">{{ number_format($product->final_price, 0, ',', ' ') }} ₽</span>@if($product->has_discount)<span class="ml-1 hidden text-xs text-slate-400 line-through sm:inline">{{ number_format($product->compare_price, 0, ',', ' ') }} ₽</span>@endif</div>
